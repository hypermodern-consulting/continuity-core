// codec_runtime.h — wire-type primitives for generated codecs
...
inline void write_u16le(std::vector<std::uint8_t>& buf, std::uint16_t v) {
  buf.resize(buf.size() + 2);
  std::memcpy(buf.data() + buf.size() - 2, &v, 2);
}

inline void write_u32le(std::vector<std::uint8_t>& buf, std::uint32_t v) {
  buf.resize(buf.size() + 4);
  std::memcpy(buf.data() + buf.size() - 4, &v, 4);
}

inline void write_u64le(std::vector<std::uint8_t>& buf, std::uint64_t v) {
  buf.resize(buf.size() + 8);
  std::memcpy(buf.data() + buf.size() - 8, &v, 8);
}

inline void write_u32be(std::vector<std::uint8_t>& buf, std::uint32_t v) {
...
inline void write_u64be(std::vector<std::uint8_t>& buf, std::uint64_t v) {
  for (int i = 7; i >= 0; i--) {
...
inline void write_varint(std::vector<uint8_t>& buf, std::uint64_t v) {
  while (v >= 128) {
