...
inline std::optional<uint8_t> read_u8(ParseState& ps) {
  if (!ps.has(1)) return std::nullopt;
  uint8_t v = ps.data[ps.pos++];
  return v;
}

inline std::optional<uint16_t> read_u16le(ParseState& ps) {
  if (!ps.has(2)) return std::nullopt;
  uint16_t v; std::memcpy(&v, ps.cur(), 2); ps.advance(2);
  return v;
}

inline std::optional<uint32_t> read_u32le(ParseState& ps) {
  if (!ps.has(4)) return std::nullopt;
  uint32_t v; std::memcpy(&v, ps.cur(), 4); ps.advance(4);
  return v;
}

inline std::optional<uint64_t> read_u64le(ParseState& ps) {
  if (!ps.has(8)) return std::nullopt;
  uint64_t v; std::memcpy(&v, ps.cur(), 8); ps.advance(8);
  return v;
}
...
inline std::optional<uint64_t> read_u64be(ParseState& ps) {
  if (!ps.has(8)) return std::nullopt;
  uint64_t v = 0;
  for (int i = 0; i < 8; i++) v = (v << 8) | ps.data[ps.pos + i];
  ps.advance(8);
  return v;
}
...
inline std::optional<std::vector<uint8_t>> read_bytes(ParseState& ps, size_t n) {
  if (!ps.has(n)) return std::nullopt;
...
inline void write_u16le(std::vector<uint8_t>& buf, uint16_t v) {
  buf.resize(buf.size() + 2); std::memcpy(buf.data() + buf.size() - 2, &v, 2);
}

inline void write_u32le(std::vector<uint8_t>& buf, uint32_t v) {
  buf.resize(buf.size() + 4); std::memcpy(buf.data() + buf.size() - 4, &v, 4);
}

inline void write_u64le(std::vector<uint8_t>& buf, uint64_t v) {
  buf.resize(buf.size() + 8); std::memcpy(buf.data() + buf.size() - 8, &v, 8);
}
...
inline void write_varint(std::vector<uint8_t>& buf, uint64_t v) {
  while (v >= 128) { buf.push_back(uint8_t(v & 0x7F) | 0x80); v >>= 7; }
