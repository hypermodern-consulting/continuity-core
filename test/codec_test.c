...
static void w_u8(Buf* b, uint8_t v) {
  buf_push(b, &v, 1);
}
static void w_u32le(Buf* b, uint32_t v) {
  buf_push(b, &v, 4);
}
static void w_u64le(Buf* b, uint64_t v) {
  buf_push(b, &v, 8);
}
...
static void w_bool64(Buf* b, int v) {
  uint64_t x = v ? 1 : 0;
  w_u64le(b, x);
}
...
static void w_varint(Buf* b, uint64_t v) {
  while (v >= 128) {
    w_u8(b, (uint8_t)(v & 0x7F) | 0x80);
    v >>= 7;
  }
  w_u8(b, (uint8_t)v);
}

static void w_bytes(Buf* b, const uint8_t* src, size_t n) {
  buf_push(b, src, n);
}
...
static uint8_t xrand_u8(void) {
  return (uint8_t)xrand();
}
static uint32_t xrand_u32(void) {
  return (uint32_t)xrand();
}
static uint64_t xrand_u64(void) {
  return xrand();
}
...
static void test_varint(void) {
  ...
  buf_init(&b);
  /* Edge cases */
  uint64_t edges[] = {0,     1,       127,       128,        16383,
                      16384, 2097151, 268435455, 0xFFFFFFFF, 0xFFFFFFFFFFFFFFFFULL};
  for (int i = 0; i < 10; i++) {
...
static void test_fuzz(void) {
  ...
  for (int i = 0; i < 100000; i++) {
    ...
    r_varint(&ps, &out64);
    ps.pos = 0;
    r_len_prefixed(&ps, outbuf, &outlen, 512);
    ps.pos = 0;
    r_bytes(&ps, outbuf, 20);
  }
  pass += 100000;
}
...
int main(void) {
  ...
  printf("  varint:         %d\n", pass);
  test_len_prefixed();
  printf("  len_prefixed:   %d\n", pass);
  test_varint_size();
  printf("  varint_size:    %d\n", pass);
  test_consumption();
  printf("  consumption:    %d\n", pass);
  test_server_hello();
  printf("  ServerHello:    %d\n", pass);
  test_pack_header();
  printf("  PackHeader:     %d\n", pass);
  test_attest_calldata();
  printf("  AttestCalldata: %d\n", pass);
  test_fuzz();
  printf("  fuzz:           %d\n", pass);
  printf("\n%d passed, %d failed\n", pass, fail_count);
