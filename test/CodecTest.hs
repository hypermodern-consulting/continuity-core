...
rngBytes n rng0 = go n rng0 []
  where
    go 0 r acc = (BS.pack (reverse acc), r)
...
testPrim name put get rng0 n genVal = go rng0 n 0
  where
    go _ 0 p = do putStrLn ("  " ++ name ++ ": " ++ show p); return p
    go r k p = do
        let (v, r') = genVal r
...
testLenPrefixed rng0 n = go rng0 n 0
  where
    go _ 0 p = do putStrLn ("  len_prefixed: " ++ show p); return p
    go r k p = do
...
testConsumption rng0 n = go rng0 n 0
  where
    go _ 0 p = do putStrLn ("  consumption: " ++ show p); return p
    go r k p = do
        let (v, r') = nextRng r
