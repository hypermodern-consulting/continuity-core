...
private def unindent4 (s : String) : String :=
  String.intercalate "\n" ((s.splitOn "\n").map (fun l => (l.drop 4).toString))
...
def cxxSFile : SFile :=
  ...
  , items :=
      ...
      , .funcDef "_run_info"
          ...
          (some "RunInfo | None")
          none
          [.raw "    return None if args == None else RunInfo(args = [args])"]
      , .blank
      , .funcDef "_llvm_toolchain_impl"
          [⟨"ctx", some "AnalysisContext", none⟩]
          (some "list[Provider]")
          (some "LLVM C++ toolchain with paths from .buckconfig.local.")
          [.raw (unindent4 cxxImplBody)]
      , .ruleDef "llvm_toolchain" "_llvm_toolchain_impl" true
          [ ("c_extra_flags", .raw "attrs.list(attrs.string(), default = [])")
          , ("cxx_extra_flags", .raw "attrs.list(attrs.string(), default = [])")
          , ("link_flags", .raw "attrs.list(attrs.string(), default = [])")
          , ("link_style", .raw "attrs.string(default = \"static\")")
...
-- #eval (renderSFile cxxSFile).length
...
def bzlFiles : List (String × String) :=
  [ ("toolchains/cxx.bzl", renderSFile cxxSFile)
