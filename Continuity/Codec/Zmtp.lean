...
namespace Continuity.Codec.Zmtp
...
def FrameFlags.fromByte (b : UInt8) : Option FrameFlags :=
  if b &&& 0xF8 != 0 then none  -- reserved bits must be zero
...
def parseFrame (bs : Bytes) : ParseResult Frame :=
  if h : bs.size > 0 then
    ...
    | none => .fail  -- reserved bits violation → reset
    | some flags =>
