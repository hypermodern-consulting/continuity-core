...
/- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      "I knew then that the ghosts were moving through the four stupid
      corners of the room, that I could feel them. The scanner was running
      through its vocabulary, eating the wall of blindness that still
      separated us, pressing the signs of its language into the soft fat
      of the sensorium, word by word, token by token, until meaning
      condensed on the far side of the scan."
...
private def isIdentByte (b : UInt8) : Bool :=
  isAlphaNum b || b == 0x5F || b == 0x2D || b == 0x2F
...
-- TODO[b7r6]: !! split this monstrosity into functions !!
...
private partial def lexOne (bs : Bytes) : Option (Tok × Bytes) :=
  ...
  if bs.size == 0 then Option.none
    ...
    -- arrow: `→` or `->`
      ...
      Option.some (Tok.arrow, bs.extract 3 bs.size)
    ...
    -- lambda: `λ` or `\`
      ...
      Option.some (Tok.kLambda, bs.extract 2 bs.size)
    ...
    -- forall: `∀`
      ...
      Option.some (Tok.kForall, bs.extract 3 bs.size)
  ...
  scanString (bs : Bytes) : Option (ByteArray × Bytes) :=
    let rec go (i : Nat) : Option (ByteArray × Bytes) :=
      if i >= bs.size then Option.none
        ...
        else if b == 0x5C && i + 1 < bs.size then
          go (i + 2)
        else go (i + 1)
    go 0
