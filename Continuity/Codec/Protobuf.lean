...
namespace Continuity.Codec.Protobuf
...
def serializeField (f : Field) : Bytes :=
  let tagBytes := Varint.serializeVarint (encodeTag f.tag)
  match f.value with
  | .varintVal v  => tagBytes ++ Varint.serializeVarint v
  | .fixed64Val v => tagBytes ++ u64le.serialize v
  | .fixed32Val v => tagBytes ++ u32le.serialize v
  | .bytesVal bs  => tagBytes ++ Varint.serializeVarint bs.size.toUInt64 ++ bs
