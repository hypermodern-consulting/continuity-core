...
def Parser.seq {T α β : Type} {k : Nat} (p1 : Parser T α k) (p2 : Parser T β k) :
    Parser T (α × β) k where
  parse ts :=
...
/-- Bind / flatMap. -/
def Parser.bind {T α β : Type} {k : Nat} (p : Parser T α k) (f : α → Parser T β k) :
    Parser T β k where
  parse ts :=
...
def Parser.orElse {T α : Type} {k : Nat} (p1 : Parser T α k) (p2 : Parser T α k) :
    Parser T α k where
  parse ts :=
