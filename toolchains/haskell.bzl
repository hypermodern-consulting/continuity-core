...
def _haskell_ffi_binary_impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    Attrs:
      hs_srcs: Haskell source files
      cxx_srcs: C/C++ source files to compile and link
      cxx_headers: C/C++ headers (for dependency tracking)
      packages: Haskell packages to link (ignored - use ghcWithPackages in Nix)
      ghc_options: Additional GHC flags
      extra_libs: External libraries to link (-l flags)
      extra_lib_dirs: Library search paths (-L flags)
      include_dirs: C/C++ include directories (-I flags)
      language_extensions: GHC language extensions
    ...
    # Add custom include directories for C++ compilation
    ...
    # Use ghc_options if provided, otherwise default to -O2 -threaded
    ...
    # Add include directories for GHC's internal C compiler (for FFI stubs)
    # Must come early so they're available during compilation
    for inc_dir in ctx.attrs.include_dirs:
        if inc_dir:  # Skip empty strings
            ghc_cmd.add("-optc", "-I" + inc_dir)
    
    if gcc_lib_base:
        ghc_cmd.add("-optl", "-L" + gcc_lib_base)
    
    # Add extra library directories
    for lib_dir in ctx.attrs.extra_lib_dirs:
        if lib_dir:  # Skip empty strings
...
haskell_ffi_binary = rule(
    ...
    attrs = {
        ...
        "deps": attrs.list(attrs.dep(), default = []),
        "packages": attrs.list(attrs.string(), default = []),  # Ignored - use ghcWithPackages
        "ghc_options": attrs.list(attrs.string(), default = []),
